...
;														not
;		1	inactive	Lightsaber inactivity		Indicates if no activity has occurred since	(1:inactive, 0:active)
;														previous start loop
;		2	onesec		1-second flag				Indicates 1 second has passed				(1:second passed, 0:second not passed)
;	
;	3) Program I/O pins:
...
;*********************************************************************************************
	...
	#define flash status0,0
	#define inactive status0,1
	#define onesec status0,2
...
initialize
  ...
  ;User Variable Initializations
	...
	movlw	second		;122 interrupts = 1 second
	movwf	msec_cnt
...
start0		;Main Program Loop
	...
	btfss 	editpb
	bra		start1
	call	check_active	;Reset counters if button was pressed
	btg		editable
	bcf		editpb
start1
	btfsc 	editable	
	...
	btfsc	editable
	call	indicate_color	;Indicate selected color
	btfsc 	flash
	call 	flash_indicate	;Flash color indicator if flash bit is enabled
	call 	output_pwm		;Output blade PWM color
	btfsc	onesec
	call	timing_check	;Update timeout counter registers
	bra		start0	

;************************************************************************************************
; Subroutine: timing_check
; Description: Decrement timeout counter registers, power down if timeout occurs
;************************************************************************************************
timing_check
	bcf		onesec	;Clear 1-second flag
	movlw	second	;Reload msec_cnt for another second of operation
	movwf	msec_cnt
	decfsz	sec_cnt	;Check to see if 1 minute has passed, if not, return to start
	return
	movlw	minute	;Reload sec_cnt for another minute of operation
	movwf	sec_cnt	
	decfsz	min_cnt	;Check to see if timeout has passed, if not, return to start
	return
	movlw	expire	;Reload min_cnt for another timeout period of operation
	movwf	min_cnt
	call	power_down
	return	
...
power_down
	...
	movff	red_level0, prev_red		;Save state of red color
	movff	green_level0, prev_green	;Save state of green color
	movff	blue_level0, prev_blue		;Save state of blue color
	movlw	0x00
...
power_wait_loop
	btfsc	colorpb		;Check to see if color selector was pressed
	bra		power_up
	btfsc	editpb		;Check to see if edit pushbutton was pressed
	bra		power_up
	bra		power_wait_loop
power_up
	movff	prev_red, red_level0		;Reactivate red led
	movff	prev_green, green_level0	;Reactivate green led
	movff	prev_blue, blue_level0		;Reactivate blue led
	call	output_pwm
...
;************************************************************************************************
; Subroutine: check_active
; Description: Check for activity, reset timeout counters if activity has occurred
;************************************************************************************************
check_active
	movlw	second
	movwf	msec_cnt
...
shift_color
	btfsc	red_flag
	movff	prev_red,red_level0
	btfsc	green_flag
	movff	prev_green,green_level0
	btfsc	blue_flag
	movff	prev_blue,blue_level0
shift_return
	return
